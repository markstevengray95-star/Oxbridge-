...
const SCHOOL_ROUTES = ["/school-dashboard","/school-overview","/human-review","/teacher-coach","/teacher-live-console","/human-interviewer"]
...
export async function updateSession(request: NextRequest) {
  ...
  if(userId&&(requiresPro||requiresSchool)&&!isAdmin){
    ...
    const paidTier=effectiveTier(subscription?.tier,subscription?.status), tier=seat?.active?"school":paidTier, hasPro=tier==="pro"||tier==="school", hasSchool=tier==="school"
