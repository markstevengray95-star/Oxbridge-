...
import { ArrowRight, BookOpenCheck, Brain, Camera, CalendarDays, ChevronDown, FileDiff, FileText, Gauge, GraduationCap, LibraryBig, MessageSquareText, Network, RefreshCw, ScanSearch, ShieldCheck, Sparkles, Target, Users, WifiOff } from "lucide-react"
...
export function NextgenTutorPanel(){
 ...
 const priorities=useMemo(()=>{
  ...
  const push=(card:ToolCard)=>{if(!cards.some(item=>item.href===card.href||item.title===card.title))cards.push(card)}
  if(due)push({title:"Oral retention check",note:`A delayed check is due on: ${due.focus}. Complete it before adding more similar practice so retention is actually tested.`,href:"/gemini-live-interview",icon:Camera,badge:"Due now",emphasis:true})
  if(!intelligence.fullPaperCount)push({title:"Build a test baseline",note:"Complete one full admissions-test paper so the Tutor can replace generic test advice with section-level evidence.",href:"/full-papers",icon:Target,badge:"Missing evidence",emphasis:true})
  else if(intelligence.priority?.domain==="Admissions test")push({title:`Target ${intelligence.priority.label}`,note:`This is currently the weakest evidenced test area at ${intelligence.priority.score}%. Use an intervention and fresh retest rather than random questions.`,href:"/paper-intervention",icon:Target,badge:"Current priority",emphasis:true})
  if(!intelligence.interviewCount)push({title:"Build an interview baseline",note:"Complete a formal live interview so reasoning aloud, flexibility and communication become part of the Tutor evidence model.",href:"/gemini-live-interview",icon:MessageSquareText,badge:"Missing evidence",emphasis:true})
  else if(intelligence.priority?.domain==="Interview")push({title:`Practise ${intelligence.priority.label}`,note:"Use a fresh interview that deliberately tests the current reasoning constraint, then compare the feedback with earlier sessions.",href:"/interview-room",icon:MessageSquareText,badge:"Current priority",emphasis:true})
  if(!intelligence.essayCount)push({title:"Build a writing baseline",note:"Analyse one substantial essay so argument structure, precision and evaluation are represented in the Tutor profile.",href:"/essay-tutor",icon:FileText,badge:"Missing evidence",emphasis:true})
  else if(intelligence.priority?.domain==="Writing")push({title:`Improve ${intelligence.priority.label}`,note:"Rewrite the weakest feature, then compare versions so the Tutor can distinguish genuine reasoning improvement from cosmetic editing.",href:"/essay-comparison",icon:FileDiff,badge:"Current priority",emphasis:true})
  if(readingCount===0)push({title:"Add academic reading evidence",note:"Record and reflect on a source so application and interview practice can draw on ideas you have actually engaged with.",href:"/source-notebook",icon:LibraryBig,badge:"Academic depth",emphasis:true})
  push({title:"Digital Twin 2.0",note:"Open the unified model of independence, transfer, retention, reading depth and application defence.",href:"/digital-twin",icon:Brain,badge:"Command centre",emphasis:true})
  push({title:"Tutor Autopilot",note:"Let the Tutor choose a session, adapt difficulty, track hint dependence and verify transfer.",href:"/tutor-autopilot",icon:Sparkles,badge:"Personalised",emphasis:true})
  push({title:"Two-person AI panel",note:"Face two adaptive interviewers with distinct AI voices and shared context.",href:"/panel-interview",icon:Users,badge:"Dual voice"})
  return cards.slice(0,3)
 },[due,intelligence,readingCount])
