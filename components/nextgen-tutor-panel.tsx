...
import { ArrowRight, BookOpenCheck, Brain, Camera, CalendarDays, ChevronDown, FileDiff, FileText, Gauge, GraduationCap, LibraryBig, RefreshCw, ScanSearch, ShieldCheck, Sparkles, Users } from "lucide-react"
...
export function NextgenTutorPanel() {
  ...
  useEffect(() => {
    ...
    try { const readings = JSON.parse(localStorage.getItem("oxbridge-reading-reflections-v1") || "[]"); if (Array.isArray(readings)) setReadingCount(readings.length) } catch {}
  }, [])
  ...
  const priorities: ToolCard[] = [
    { title: "Tutor Autopilot", note: "Let the Tutor choose the session, adapt difficulty, track hint dependence and check transfer.", href: "/tutor-autopilot", icon: Brain, badge: "Personalised", emphasis: true },
    due
    ...
    { title: "Two-person AI panel", note: "Face two adaptive interviewers with distinct AI voices and shared context.", href: "/panel-interview", icon: Users, badge: "Dual voice" },
  ]
  ...
  const library: Array<{ heading: string; items: ToolCard[] }> = [
    ...
    },
    {
      ...
      items: [
        { title: "Reading Curriculum", note: "Follow a 6–12 week pathway that returns to ideas and forces synthesis between readings.", href: "/reading-curriculum", icon: LibraryBig, badge: "Curriculum" },
        ...
        { title: "Written Work Defence", note: "Map the claims and assumptions in your own work, then defend them in interview.", href: "/written-work-defence", icon: FileText, badge: "Application" },
        { title: "Essay Version Comparison", note: "Compare draft-to-draft changes and separate real reasoning improvement from cosmetic rewriting.", href: "/essay-comparison", icon: FileDiff, badge: "Writing" },
    ...
    {
      heading: "Application & quality",
      items: [
        { title: "Application Command Centre", note: "Connect course, College preference, written work, tests, reading and interview preparation.", href: "/application-command-centre", icon: GraduationCap, badge: "Pathway" },
  ...
  return <section className="mx-auto max-w-7xl px-4 pb-28 sm:px-6 md:pb-10">
    <div className="mb-4 flex flex-wrap items-end justify-between gap-3"><div><p className="text-xs font-bold uppercase tracking-[.18em] text-[#147d91]">Tutor recommendations</p><h2 className="mt-1 font-serif text-2xl font-bold text-[#172b3a]">Do the next useful thing, not everything at once.</h2><p className="mt-1 max-w-2xl text-sm text-[#667984]">Only the highest-value actions stay visible. The deeper toolset is grouped below so the Tutor remains calm and focused.</p></div></div>
    <div className="grid gap-4 lg:grid-cols-3">{priorities.map(item => <Card key={item.title} className={item.emphasis ? "border-[#8dd7de] bg-[#f4fbfb] shadow-none" : "shadow-none"}><CardHeader><div className="flex items-start justify-between gap-3"><item.icon className="size-5 text-[#147d91]" /><Badge variant={item.emphasis ? "default" : "outline"}>{item.badge}</Badge></div><CardTitle className="font-serif text-xl">{item.title}</CardTitle><CardDescription>{item.note}</CardDescription></CardHeader><CardContent><Button asChild size="sm" variant={item.emphasis ? "default" : "outline"}><Link href={item.href}>Start <ArrowRight /></Link></Button></CardContent></Card>)}</div>
    <details className="group mt-5 rounded-2xl border bg-white"><summary className="flex cursor-pointer list-none items-center justify-between gap-4 p-5"><div><p className="font-semibold text-[#172b3a]">Explore the full preparation studio</p><p className="mt-1 text-sm text-[#667984]">Open this when you want to choose a specific tool manually.</p></div><ChevronDown className="size-5 text-[#667984] transition-transform group-open:rotate-180" /></summary><div className="grid gap-6 border-t p-5 lg:grid-cols-3">{library.map(group => <div key={group.heading}><p className="mb-3 text-xs font-bold uppercase tracking-[.15em] text-[#147d91]">{group.heading}</p><div className="space-y-3">{group.items.map(item => <Link key={item.title} href={item.href} className="block rounded-xl border p-4 transition-colors hover:border-[#8dd7de] hover:bg-[#f8fbfb]"><div className="flex items-start gap-3"><item.icon className="mt-0.5 size-4 shrink-0 text-[#147d91]" /><div className="min-w-0"><div className="flex flex-wrap items-center gap-2"><strong className="text-sm text-[#172b3a]">{item.title}</strong><Badge variant="outline" className="text-[10px]">{item.badge}</Badge></div><p className="mt-1 text-xs leading-5 text-[#667984]">{item.note}</p></div></div></Link>)}</div></div>)}</div></details>
  </section>
}
