...
export function ReviewPanel({ essay, mode, prompt = "", test = "Essay", course = "" }: { essay: string; mode: WritingMode; prompt?: string; test?: string; course?: string }) {
  ...
  function localResult(message: string): Result {
    ...
    return { provider: "local", report, strictScore: mode === "essay" ? scoreStrictEssay(report, prompt, essay) : null, mechanics: mechanics(essay), message, rubricVersion: 5 }
  }
  ...
  function acceptResult(result: Result) {
    ...
    try {
      ...
      const classification = result.strictScore?.classification ?? result.strictScore?.grade
      const item = { id: `writing-${Date.now()}`, test, prompt, essay, course, mode, rubricVersion: result.rubricVersion, provider: result.provider, overall, classification, grade: classification, strictScore: result.strictScore, weakest: result.report.priorities[0]?.title, report: result.report, analysis: { dimensions, priorityImprovements: result.report.priorities.map(p => p.action) }, date: new Date().toISOString() }
  ...
  function download() {
    ...
    const scoreLines = data.strictScore ? ["", `University-style practice mark: ${data.strictScore.score}/100`, `Practice classification: ${data.strictScore.classification} — ${data.strictScore.descriptor}`, `Raw weighted mark: ${data.strictScore.rawScore}/100`, ...data.strictScore.components.map(c => `${c.label}: ${c.earned}/${c.weight}`), ...(data.strictScore.caps.length ? ["Classification ceilings:", ...data.strictScore.caps.map(c => `Maximum ${c.maximum}/100 — ${c.reason}`)] : ["Classification ceilings: none triggered"]), data.strictScore.note, ""] : []
