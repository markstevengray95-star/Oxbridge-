...
import { LEVELS, splitParagraphs, type WritingMode, type WritingReport } from "@/lib/writing/review"

type Result = { provider: "gemini" | "local"; report: WritingReport | null; message?: string; mechanics: { words: number; characters: number; paragraphs: number; checks: { paragraph: number; quote: string; message: string }[] }; rubricVersion: number }
...
export function ReviewPanel({ essay, mode, prompt = "", test = "Essay", course = "" }: { essay: string; mode: WritingMode; prompt?: string; test?: string; course?: string }) {
  ...
  async function review() {
    ...
    setBusy(true); setError(""); setSaved(null); setCompleted([])
    try {
      ...
      if (result.report) {
        try {
          ...
          const overall = dimensions.length ? Math.round(dimensions.reduce((a,c) => a + c.score, 0) / dimensions.length) : undefined
          const item = { id: `writing-${Date.now()}`, test, prompt, essay, course, mode, rubricVersion: 2, provider: "gemini", overall, weakest: result.report.priorities[0]?.title, report: result.report, analysis: { dimensions, priorityImprovements: result.report.priorities.map(p => p.action) }, date: new Date().toISOString() }
          ...
          setSaveStatus("Review saved on this device.")
        } catch { setSaveStatus("Review is displayed, but device storage is unavailable. Download the report to keep it.") }
      }
    } catch (e) { if (!abort.signal.aborted) setError(e instanceof Error ? e.message : "Analysis failed. Please retry.") } finally { if (!abort.signal.aborted) setBusy(false) }
  }
  function download() {
    if (!report) return
    const lines = [mode === "essay" ? "Essay review" : "Personal statement review", prompt || course, report.summary, "", ...report.criteria.flatMap(c => [c.label + ': ' + (c.level === null ? "Not assessed" : LEVELS[c.level]), c.judgement, c.evidence.quote ? `P${c.evidence.paragraph!+1}: "${c.evidence.quote}"` : "Whole-draft observation", "Action: " + c.action, ""]), "Revision priorities", ...report.priorities.flatMap((p,i) => [`${i+1}. ${p.title}`, p.why, p.action, "Success check: " + p.successCheck]), "", "Paragraph review", ...report.paragraphs.flatMap(p => [`P${p.index+1}: ${p.purpose}`, "Strength: " + p.strength, "Limitation: " + p.limitation, "Action: " + p.action]), "", "Passage annotations", ...report.annotations.flatMap(a => [`P${a.evidence.paragraph!+1}: "${a.evidence.quote}"`, a.explanation, a.revision]), "", "Follow-up questions", ...report.questions.map(q => q.question + " — " + q.purpose), "", ...report.limitations, "", "Original draft", essay]
  ...
  return <section className="space-y-5" aria-label="Detailed writing review">
    <div className="rounded-2xl border bg-white p-5"><div className="flex flex-wrap items-center justify-between gap-3"><div><h2 className="font-serif text-2xl font-bold">Evidence-based {mode === "essay" ? "essay" : "statement"} analysis</h2><p className="mt-1 text-sm text-slate-600">Specific passages, explained judgements and a practical revision plan.</p></div><Button onClick={() => void review()} disabled={busy || essay.trim().length < 40 || essay.length > 20000}>{busy ? "Reading and checking evidence…" : report ? "Analyse again" : "Analyse writing"}</Button></div><p className="mt-3 text-xs text-slate-500">Submitting sends this draft to Gemini for analysis. Reviews are coaching guidance, not official marks, fact checking or an admissions prediction.</p>{error && <p role="alert" className="mt-3 text-sm text-red-800">{error}</p>}{saved && !data && <p className="mt-3 text-sm text-amber-800">Your draft or task has changed. Run a new review to update the feedback.</p>}</div>
    {data?.message && <div role="status" className="rounded-xl border border-amber-200 bg-amber-50 p-4 text-sm">{data.message}</div>}
    {data && <div className="rounded-xl bg-slate-50 p-4 text-sm"><strong>Draft facts:</strong> {data.mechanics.words} words · {data.mechanics.characters} characters · {data.mechanics.paragraphs} paragraphs{!report && <p className="mt-2">No quality score is available. {data.mechanics.checks.length ? "Sentence-length checks follow." : "No long-sentence flags found; this does not assess reasoning or content."}</p>}{!report && data.mechanics.checks.map((c,i) => <div className="mt-3" key={i}><Evidence paragraph={c.paragraph} quote={c.quote}/><p>{c.message}</p></div>)}</div>}
    {report && <>
      <div className="rounded-2xl bg-[#102a43] p-6 text-white"><h2 className="font-serif text-2xl font-bold">What this draft achieves</h2><p className="mt-3 whitespace-pre-wrap leading-7">{report.summary}</p><p className="mt-3 text-sm text-slate-200">{saveStatus}</p><Button className="mt-3 bg-white text-slate-900 hover:bg-slate-100" onClick={download}>Download full report</Button></div>
