...
function fallback(body: RequestBody) {
  ...
  if (body.mode === "evaluate") {
    ...
    return {
      ...
      whatWorked: words > 35 ? "You developed an answer rather than stopping at a conclusion." : "You committed to an answer and created something that can be tested.",
      missed: `Make the reasoning around ${target} more explicit and test one alternative.` ,
      transferScore: Math.max(30, Math.round(score - 5)),
      nextQuestion: "What would have to change for your conclusion to stop being true?",
      hintLevelSuggestion: score >= 70 ? "independent" : score >= 55 ? "light" : "substantial",
    }
  }
  return {
    question: `${difficulty} ${course} challenge: take a familiar principle from your subject and apply it to an unfamiliar case. State your first model, identify one assumption, and explain what evidence would make you revise it.`,
    successCriteria: ["Make the reasoning chain visible", `Deliberately practise ${target}`, "Test an assumption or limiting case", "Finish with a provisional conclusion"],
    transferTwist: "Now change one important condition and explain which parts of your original reasoning survive.",
    targetSkill: target,
  }
}
  ...
  for (const candidate of keys) {
    try {
      const response = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/${encodeURIComponent(model)}:generateContent`, {
        ...
        body: JSON.stringify({ systemInstruction: { parts: [{ text: system }] }, contents: [{ role: "user", parts: [{ text: user }] }], generationConfig: { temperature: mode === "generate" ? 0.7 : 0.2, maxOutputTokens: 1800, responseMimeType: "application/json" } }),
