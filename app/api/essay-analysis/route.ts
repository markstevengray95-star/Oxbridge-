...
export async function POST(request: Request) {
  ...
  const fallback = (message: string) => NextResponse.json({ provider: "local", report: null, mechanics: basic, message, rubricVersion: 2 })
  const key = getGeminiApiKeyCandidates()[0]?.value
  if (!key) return fallback("AI review is not configured on this deployment. These are mechanical checks only; argument quality has not been assessed.")
  try {
    ...
    if (!response.ok) return fallback("AI review is temporarily unavailable. Your draft is unchanged. Retry for a substantive review.")
    const data = await response.json() as { candidates?: { finishReason?: string; content?: { parts?: { text?: string; thought?: boolean }[] } }[] }
    const candidate = data.candidates?.[0]
    if (candidate?.finishReason !== "STOP") return fallback("The AI review was incomplete. No partial scores are shown; please retry.")
    ...
    return NextResponse.json({ provider: "gemini", report, mechanics: basic, rubricVersion: 2 })
  } catch {
    return fallback("The review could not be verified against your draft or timed out. No unverified feedback or scores are shown. Please retry.")
  }
}
