...
function modeInstruction(mode: TutorMode) {
  ...
  if (mode === "review") return "Act as a performance reviewer. Identify patterns across evidence, distinguish one-off errors from recurring behaviours, and prescribe one measurable next intervention."
...
async function loadCloudContext(supabase: SupabaseServerClient, userId: string): Promise<CloudContext> {
  ...
  return {
    intelligence: intelligence.data?.snapshot ?? {},
    plans: plans.data ?? [],
    memories: memories.data ?? [],
    mistakes: mistakes.data ?? [],
    evidence: evidence.data ?? [],
    application: application.data ?? [],
    supercurricular: supercurricular.data ?? [],
    chat: cleanConversation(chatState?.messages),
    reflection: typeof reflectionState?.reflection === "string" ? reflectionState.reflection.slice(0, 2500) : "",
  }
}
...
async function persistTutorState(
  ...
  if (reflection.trim()) {
    rows.push({
      ...
      state_value: { reflection: reflection.trim().slice(0, 2500) },
      updated_at: now,
    })
  }
...
export async function POST(request: Request) {
  ...
  if (!userId) {
    return NextResponse.json({ reply: local, provider: "local", authRequired: true, mode })
  }
  ...
  const system = `You are the student's persistent Oxbridge preparation tutor. You receive evidence from their authenticated cloud preparation history. Use it to decide what is most useful next.
...
- Tie advice to supplied evidence; do not invent marks, experiences, books, projects, weaknesses or application facts.
...
- Distinguish weak evidence from strong repeated patterns.
...
- Notice recurring mistake patterns, spaced retests, neglected areas and previous reflections.
- Use concise professional British English suitable for a secondary-school applicant.
- If there is little evidence, explicitly identify the baseline activity that would give the tutor useful information.
- Treat stored memories as evidence with uncertainty, not unquestionable facts.
- End most coaching/challenge responses with one focused next question or action.`

  const compactCloud = JSON.stringify({
    cloudIntelligence: cloud.intelligence,
    recentPlans: cloud.plans,
    activeMemories: cloud.memories,
    recentMistakes: cloud.mistakes,
    progressEvidence: cloud.evidence,
    applicationEvidence: cloud.application,
    supercurricular: cloud.supercurricular,
