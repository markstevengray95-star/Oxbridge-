...
function localAnalysis(exchanges: Exchange[]): StructuredSession {
  const answers = exchanges.map((exchange, index) => {
    const answer = (exchange.answer ?? "").trim()
    const feedback = analyseInterviewAnswer(answer)
    return {
      index,
      question: (exchange.question ?? "Interview question").trim(),
      answer,
      ...feedback,
    }
  })
  ...
  for (const item of answers) {
    targetCounts.set(item.dominantTarget, (targetCounts.get(item.dominantTarget) ?? 0) + 1)
    item.strengths.forEach(strength => strengthCounts.set(strength, (strengthCounts.get(strength) ?? 0) + 1))
  }
  ...
  return {
    overallSummary: `Across ${answers.length} answer${answers.length === 1 ? "" : "s"}, the current practice profile averages ${average}/100. The review focuses on visible reasoning behaviour rather than admissions prediction.`,
    recurringStrengths: recurringStrengths.length ? recurringStrengths : ["You stayed engaged with the questions and produced analysable reasoning."],
    recurringWeaknesses: recurringWeaknesses.length ? recurringWeaknesses : ["Make more of the reasoning chain explicit."],
    priorityTarget: recurringWeaknesses[0] ?? "Reasoning chain",
    nextInterviewPlan: [
      "Answer the first question by stating the key observation and governing principle before concluding.",
      "Name one assumption explicitly and test what happens if it changes.",
      "Use one counterexample, limiting case or discriminating piece of evidence before finalising the answer.",
    ],
    answers,
  }
}
...
function parseJson(raw: string) {
  ...
  try { return JSON.parse(cleaned) as StructuredSession } catch { return null }
}
...
export async function POST(request: Request) {
  ...
  const exchanges = (Array.isArray(body.exchanges) ? body.exchanges : [])
    .map(item => ({ question: (item.question ?? "").slice(0, 1800), answer: (item.answer ?? "").slice(0, 5000) }))
  ...
  const system = `You are an Oxford/Cambridge interview-practice coach analysing a secondary-school applicant's answers. Do not predict admissions outcomes and do not pretend to use an official university scoring rubric. Analyse only observable reasoning in the supplied answers. Be specific, evidence-based, demanding and constructive. Return ONLY valid JSON with this exact shape: {"overallSummary":string,"recurringStrengths":string[],"recurringWeaknesses":string[],"priorityTarget":string,"nextInterviewPlan":string[],"answers":[{"index":number,"question":string,"answer":string,"score":number,"strengths":string[],"improvements":string[],"dimensions":[{"label":string,"score":number,"evidence":string,"action":string}],"nextMove":string,"dominantTarget":string}]}. Each answer score is a practice signal 0-100. Use these dimensions where relevant: reasoning chain, subject/relevance, assumptions, evidence/testing alternatives, precision, adaptability and communication. Quote or closely reference specific reasoning moves from the candidate, but keep quotations short. Give 2-3 strengths and 2-3 improvements per answer. Analyse every supplied answer. Question and answer fields must be copied from the supplied exchanges.`
  ...
  try {
    const response = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/${encodeURIComponent(model)}:generateContent`, {
      ...
      body: JSON.stringify({
        ...
        generationConfig: { temperature: 0.25, maxOutputTokens: 5000, responseMimeType: "application/json" },
      }),
