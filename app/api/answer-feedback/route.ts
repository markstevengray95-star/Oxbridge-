...
type FeedbackRequest = { test?: string; section?: string; difficulty?: string; prompt?: string; options?: string[]; selectedAnswer?: string; correctAnswer?: string; explanation?: string; correct?: boolean }
...
async function loadStudentContext() {
  ...
  } catch { return "No saved learner history is available." }
}
...
function fallback(body: FeedbackRequest) {
  ...
  return {
    ...
    personalisedFeedback: correct ? `You selected the correct answer in ${body.section || "this section"}. Identify the decisive step so you can reproduce it on a less familiar version.` : "Compare your choice with the correct option and identify the first point where the reasoning diverged; that is more useful than memorising the final answer.",
    whyThisChoice: correct ? "Your answer is consistent with the key reasoning described in the explanation." : "The selected option matches part of the problem but misses or misapplies a decisive condition captured by the correct answer.",
    patternConnection: "Connect this attempt to a recurring academic pattern such as interpretation, assumptions, calculation setup, timing or distractor selection.",
...
export async function POST(request: Request) {
  ...
  } catch { return NextResponse.json({ feedback: local, provider: "local", degraded: true }) }
}
