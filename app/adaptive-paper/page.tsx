...
export default function AdaptivePaperPage(){
  ...
  const bank=useMemo(()=>legacyHardenedQuestionBank.filter(q=>q.test===test),[test])
  ...
  return <main className="min-h-screen bg-[#f6f8f8] text-[#172b3a]">
    <header className="border-b bg-white"><div className="mx-auto flex max-w-6xl items-center justify-between px-4 py-4 sm:px-6"><Button asChild variant="ghost"><Link href="/student-home"><ArrowLeft/>Student Home</Link></Button><Badge variant="outline"><Target className="size-3.5"/>Adaptive paper</Badge></div></header><div className="mx-auto max-w-6xl px-4 py-8 sm:px-6">
      ...
      <Card className="shadow-none"><CardHeader><div className="flex items-center justify-between gap-3"><div><CardTitle className="font-serif text-2xl">{test} adaptive paper</CardTitle><CardDescription>{paper.length} questions · seed {seed}</CardDescription></div><Button variant="outline" onClick={regenerate}><RefreshCw/>Fresh paper</Button></div></CardHeader><CardContent className="space-y-5">{paper.map((q,i)=><div key={q.id} className="rounded-2xl border bg-white p-4"><div className="mb-3 flex flex-wrap gap-2"><Badge>{i+1}</Badge><Badge variant="outline">{q.section}</Badge><Badge variant="outline">{q.difficulty}</Badge>{!exposure.includes(q.id)&&<Badge variant="outline">Unseen</Badge>}</div><p className="font-serif text-lg leading-relaxed">{q.prompt}</p><div className="mt-3 grid gap-2">{q.options.map((opt,j)=>{const chosen=answers[q.id]===j;const cls=submitted?(j===q.answer?"border-emerald-500 bg-emerald-50":chosen?"border-red-400 bg-red-50":""):chosen?"border-[#147d91] bg-[#edf7f8]":"";return <button disabled={submitted} key={`${q.id}-${j}`} onClick={()=>setAnswers(a=>({...a,[q.id]:j}))} className={`rounded-xl border p-3 text-left text-sm ${cls}`}><strong className="mr-2">{String.fromCharCode(65+j)}.</strong>{opt}{submitted&&j===q.answer&&<Check className="ml-2 inline size-4 text-emerald-700"/>}</button>})}</div>{submitted&&<p className="mt-3 text-sm leading-relaxed text-slate-600">{q.explanation}</p>}</div>)}</CardContent></Card>
