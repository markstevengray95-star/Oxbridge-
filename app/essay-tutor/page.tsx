...
export default function EssayTutorPage() {
  ...
  return <main className="min-h-screen bg-[#f6f8f8] text-[#172b3a]">
    ...
    <div className="mx-auto max-w-7xl px-4 py-8 sm:px-6">
      <section className="mb-6 flex flex-col justify-between gap-4 lg:flex-row lg:items-end"><div><p className="text-xs font-bold uppercase tracking-[.18em] text-[#147d91]">LNAT / TARA argument writing</p><h1 className="mt-2 font-serif text-4xl font-bold">Analyse the argument paragraph by paragraph.</h1><p className="mt-2 max-w-3xl text-slate-600">The review now identifies strong sections, improvement zones, the thesis, reasoning chain, counterargument, qualification, paragraph roles and a detailed rewrite plan. Practice scores are diagnostic signals, not official test marks.</p></div><div className="flex gap-2">{(["LNAT", "TARA"] as Test[]).map(x => <Button key={x} variant={test === x ? "default" : "outline"} onClick={() => { setTest(x); setEssay(""); setReviewed(false); setDetailed(null); setProvider(null) }}>{x}</Button>)}</div></section>

      <section className="grid gap-5 xl:grid-cols-[1.1fr_.9fr]"><Card className="shadow-none"><CardHeader><div className="flex flex-wrap gap-2"><Badge>{test}</Badge><Badge variant="outline">Practice prompt</Badge></div><CardTitle className="font-serif text-2xl leading-snug">{prompt}</CardTitle><CardDescription>Plan briefly, state a defensible position, develop reasons, answer the strongest objection and conclude without simply repeating the introduction.</CardDescription></CardHeader><CardContent className="space-y-4"><Textarea rows={22} value={essay} onChange={e => { setEssay(e.target.value); setReviewed(false); setDetailed(null) }} placeholder="Write your response here… Use blank lines between paragraphs for the clearest paragraph-level analysis." /><div className="flex flex-wrap items-center justify-between gap-3"><span className="text-sm text-slate-500">{essay.trim() ? essay.trim().split(/\s+/).length : 0}{test === "TARA" ? " / 750" : ""} words · {paragraphs.length} paragraph{paragraphs.length === 1 ? "" : "s"}</span><div className="flex gap-2"><Button variant="outline" onClick={nextPrompt}><RefreshCw />New prompt</Button><Button onClick={reviewEssay} disabled={!essay.trim() || loading}>{loading ? <Loader2 className="animate-spin" /> : <Target />} {loading ? "Analysing…" : "Full analysis"}</Button></div></div></CardContent></Card>

        <aside className="space-y-4">{reviewed ? <><Card className="border-0 bg-[#102a43] text-white shadow-none"><CardHeader><div className="flex items-center justify-between"><div><CardDescription className="text-blue-50/70">Practice argument signal</CardDescription><CardTitle className="font-serif text-5xl">{overall}<span className="text-xl text-blue-100/60">/100</span></CardTitle></div>{provider && <Badge className="bg-white/10 text-white">{provider === "gemini" ? "AI detailed review" : "Built-in review"}</Badge>}</div><CardDescription className="text-blue-50/70">Not an official LNAT/TARA score.</CardDescription></CardHeader></Card><Card className="shadow-none"><CardHeader><CardTitle className="font-serif text-xl">Argument diagnostics</CardTitle></CardHeader><CardContent className="space-y-4">{diagnostics.map(d => <div key={d.label}><div className="mb-1 flex justify-between text-sm"><strong>{d.label}</strong><span>{Math.round(d.score)}%</span></div><Progress value={d.score} /><p className="mt-1 text-xs text-slate-500">{d.note}</p></div>)}</CardContent></Card><Card className="border-amber-200 bg-amber-50 shadow-none"><CardHeader><CardDescription>First revision priority</CardDescription><CardTitle className="font-serif text-xl">{weakest.label}</CardTitle></CardHeader><CardContent><p className="text-sm leading-relaxed">{weakest.next}</p></CardContent></Card></> : <Card className="shadow-none"><CardHeader><CardTitle className="font-serif text-xl">What the full review checks</CardTitle></CardHeader><CardContent className="space-y-2 text-sm text-slate-600"><p>• Thesis and directness of answer</p><p>• Logical chain between claims</p><p>• Counterargument and response</p><p>• Qualification and exceptions</p><p>• Use of examples and evidence</p><p>• Paragraph function and transitions</p><p>• Conclusion and overall coherence</p></CardContent></Card>}</aside></section>
      ...
      {reviewed && detailed && <div className="mt-7 space-y-6">
        ...
        <Card className="shadow-none"><CardHeader><CardTitle className="font-serif text-2xl">Highlighted essay</CardTitle><CardDescription>Green sections are currently doing a strong argumentative job. Amber sections have useful material but need sharpening. Red sections are the clearest improvement opportunities.</CardDescription></CardHeader><CardContent className="space-y-4">{paragraphs.map((paragraph, index) => { const review = detailed.paragraphs.find(item => item.index === index); const status = review?.status ?? "mixed"; const style = status === "strong" ? "border-emerald-300 bg-emerald-50" : status === "improve" ? "border-rose-200 bg-rose-50" : "border-amber-200 bg-amber-50"; return <div key={`${index}-${paragraph.slice(0, 20)}`} className={`rounded-2xl border p-5 ${style}`}><div className="mb-3 flex flex-wrap items-center justify-between gap-2"><div className="flex gap-2"><Badge variant="outline">Paragraph {index + 1}</Badge><Badge>{review?.role ?? "Development"}</Badge></div><strong className="text-xs uppercase tracking-wider">{status === "strong" ? "Strong section" : status === "improve" ? "Improve" : "Mixed / developing"}</strong></div><p className="text-sm leading-7">{paragraph}</p>{review && <div className="mt-4 grid gap-3 md:grid-cols-2"><div className="rounded-xl bg-white/70 p-3"><p className="text-xs font-bold uppercase tracking-wider text-emerald-800">What works</p><p className="mt-1 text-sm leading-6">{review.whatWorks}</p></div><div className="rounded-xl bg-white/70 p-3"><p className="text-xs font-bold uppercase tracking-wider text-amber-800">Improve</p><p className="mt-1 text-sm leading-6">{review.improve}</p><p className="mt-2 text-xs text-slate-500"><strong>Revision action:</strong> {review.action}</p></div></div>}</div>})}</CardContent></Card>
