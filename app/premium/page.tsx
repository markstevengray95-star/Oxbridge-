...
const tiers = [
  ...
  },
  {
    ...
    name: "Pro",
    icon: Sparkles,
    note: "The full individual preparation system for an Oxford or Cambridge applicant.",
...
export default async function PremiumPage({ searchParams }: PageProps) {
  ...
  const notice = params.billing === "cancelled"
    ...
    : params.billing === "success"
      ...
      : params.billing === "stripe-not-configured"
        ? "Payments are not connected on this Vercel project yet. Add STRIPE_SECRET_KEY in Vercel, then redeploy."
        : params.billing === "supabase-not-configured"
          ? "Payments cannot start because the Supabase server secret is missing in Vercel. Add SUPABASE_SECRET_KEY, then redeploy."
          : params.billing === "database-not-ready"
            ? "Payments are connected, but the new Supabase project is missing the billing tables. Restore the billing schema, then try again."
            : params.billing === "checkout-error"
              ? "Stripe Checkout could not start. Check the Vercel Stripe and Supabase settings, then try again."
  ...
  const billingError = ["stripe-not-configured", "supabase-not-configured", "database-not-ready", "checkout-error"].includes(params.billing || "")
  ...
  return <main className="min-h-screen bg-[#f5f7f7] text-[#172b3a]">
    ...
    <div className="mx-auto max-w-7xl space-y-8 px-4 py-10 sm:px-6">
      <section className="text-center"><Badge variant="outline">Step 2 of 2 · Choose your plan</Badge><h1 className="mx-auto mt-4 max-w-4xl font-serif text-4xl font-bold sm:text-5xl">Choose how you want to use ScholarBridge.</h1><p className="mx-auto mt-4 max-w-3xl text-slate-600">Every user creates an account first. Select Free, Pro or School below to unlock the app. Prices, included features and monthly Live usage are shown before you continue.</p>{notice && <div className={`mx-auto mt-5 max-w-2xl rounded-2xl border p-4 text-sm ${billingError ? "border-red-200 bg-red-50 text-red-900" : "border-[#b9d8dd] bg-[#edf7f8] text-[#234754]"}`}>{notice}</div>}{hasChosen && <div className="mx-auto mt-5 max-w-2xl rounded-2xl border border-emerald-200 bg-emerald-50 p-4 text-sm text-emerald-950">Your account already has a plan choice. <Button asChild size="sm" className="ml-2"><Link href="/student-home">Enter the app</Link></Button></div>}</section>
