...
export default async function PremiumPage({ searchParams }: PageProps) {
  ...
  return <main className="min-h-screen bg-[#f5f7f7] text-[#172b3a]">
    ...
    <div className="mx-auto max-w-7xl space-y-8 px-4 py-10 sm:px-6">
      <section className="text-center"><Badge variant="outline">Step 2 of 2 · Choose your plan</Badge><h1 className="mx-auto mt-4 max-w-4xl font-serif text-4xl font-bold sm:text-5xl">Choose how you want to use Oxbridge Tutor.</h1><p className="mx-auto mt-4 max-w-3xl text-slate-600">Every user creates an account first. Select Free, Pro or School below to unlock the app. Prices, included features and monthly Live usage are shown before you continue.</p>{notice && <div className={`mx-auto mt-5 max-w-2xl rounded-2xl border p-4 text-sm ${billingError ? "border-red-200 bg-red-50 text-red-900" : "border-[#b9d8dd] bg-[#edf7f8] text-[#234754]"}`}>{notice}</div>}{hasChosen && <div className="mx-auto mt-5 max-w-2xl rounded-2xl border border-emerald-200 bg-emerald-50 p-4 text-sm text-emerald-950">Your account already has a plan choice. <Button asChild size="sm" className="ml-2"><Link href="/student-home">Enter the app</Link></Button></div>}</section>
