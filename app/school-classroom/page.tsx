...
import { AssignmentReflection } from "@/components/school/assignment-reflection"
...
export default function SchoolClassroomPage() {
  ...
  async function update(assignmentId: string, status: OwnProgress["status"]) {
    try { const saved = data?.joined.flatMap(item => item.progress).find(row => row.assignment_id === assignmentId); await post({ action: "updateAssignmentStatus", assignmentId, status, note: saved?.note ?? "" }); await load() } catch (cause) { setError(cause instanceof Error ? cause.message : "Could not update assignment") }
  }
