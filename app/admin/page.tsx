...
import { BarChart3, Brain, GraduationCap, KeyRound, Mic2, ShieldCheck, Sparkles, Users } from "lucide-react"
...
export default async function AdminPage() {
  ...
  return <main className="min-h-screen bg-[#eef3f4] px-4 py-7 text-[#172b3a] sm:px-6 lg:px-8 lg:py-10"><div className="mx-auto max-w-7xl space-y-6">
    <div className="flex flex-wrap items-center justify-between gap-4"><div><div className="flex items-center gap-2"><Badge className="bg-[#102a43] text-white"><ShieldCheck className="size-3.5" />Administrator</Badge><Badge variant="outline">Unlimited access</Badge></div><h1 className="mt-3 font-serif text-4xl font-bold">Oxbridge Admin Console</h1><p className="mt-2 max-w-2xl text-sm leading-6 text-[#667984]">Full app access, unlimited Gemini Live usage, account monitoring and a high-level view of the preparation platform.</p></div><div className="flex gap-2"><Button asChild variant="outline"><Link href="/account">Account</Link></Button><Button asChild><Link href="/tutor"><Brain />Open Tutor</Link></Button></div></div>
    ...
    <section className="grid gap-5 lg:grid-cols-[1.05fr_.95fr]"><Card className="border-emerald-200 bg-emerald-50"><CardHeader><CardTitle className="flex items-center gap-2 font-serif text-2xl"><ShieldCheck className="size-5 text-emerald-700" />Admin access is active</CardTitle><CardDescription>Your administrator role is verified on the server.</CardDescription></CardHeader><CardContent className="space-y-3 text-sm leading-6 text-emerald-950"><p>Gemini Live session reservations are bypassed for this account, so the normal monthly minute cap does not apply.</p><p>All Pro and School route gates are bypassed while the account remains an administrator.</p><p>Password management uses secure reset emails: administrators can never view a user’s password.</p><p className="text-xs text-emerald-800">Role source: {access.source === "database" ? "protected Supabase admin table" : "server environment allowlist"}.</p></CardContent></Card>

      <Card><CardHeader><CardTitle className="font-serif text-2xl">Admin shortcuts</CardTitle><CardDescription>Manage users or jump directly into the areas you are testing.</CardDescription></CardHeader><CardContent className="grid gap-2 sm:grid-cols-2"><Button asChild><Link href="/admin/users"><KeyRound />Users & passwords</Link></Button><Button asChild variant="outline"><Link href="/gemini-live-interview"><Mic2 />Gemini Live</Link></Button><Button asChild variant="outline"><Link href="/tutorial-lab">Tutorial Lab</Link></Button><Button asChild variant="outline"><Link href="/school-dashboard">School dashboard</Link></Button><Button asChild variant="outline"><Link href="/human-review">Human review</Link></Button><Button asChild variant="outline"><Link href="/premium">Premium overview</Link></Button></CardContent></Card></section>
  </div></main>
}
