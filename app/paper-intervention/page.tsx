...
type InterventionResult = { id: string; sourcePaperId: string; test: string; originalAccuracy: number; retestAccuracy: number; targets: string[]; date: string }
...
export default function PaperInterventionPage() {
  ...
  const latestRetest = history[0]?.retestAccuracy ?? null
  ...
  return <main className="min-h-screen bg-[#f5f7f7] text-[#172b3a]">
    <header className="border-b bg-white"><div className="mx-auto flex max-w-7xl items-center justify-between px-4 py-4"><Button asChild variant="ghost"><Link href="/full-papers"><ArrowLeft />Full Papers</Link></Button><Badge variant="outline"><TrendingUp className="size-3.5" />Weakness → intervention → retest → mastery</Badge></div></header>
    <div className="mx-auto max-w-7xl space-y-6 px-4 py-8">
      <section className="grid gap-5 lg:grid-cols-[1.15fr_.85fr]"><div><p className="text-xs font-bold uppercase tracking-[.18em] text-[#147d91]">Automatic intervention</p><h1 className="mt-2 font-serif text-4xl font-bold">Turn the last paper into the next learning cycle.</h1><p className="mt-3 max-w-3xl text-slate-600">The app ranks the weakest sections, gives a short targeted teaching sequence, builds a fresh retest and then tracks whether performance is genuinely becoming secure over repeated attempts.</p></div><div className="grid gap-3 sm:grid-cols-3"><Card className="border-0 bg-[#102a43] text-white shadow-none"><CardHeader><CardDescription className="text-white/60">Paper</CardDescription><CardTitle className="font-serif text-3xl">{latest.accuracy}%</CardTitle><CardDescription className="text-white/60">{latest.rawScore}/{latest.totalQuestions}</CardDescription></CardHeader></Card><Card className="shadow-none"><CardHeader><CardDescription>Best retest</CardDescription><CardTitle className="font-serif text-3xl">{bestRetest === null ? "—" : `${bestRetest}%`}</CardTitle></CardHeader></Card><Card className={mastery === "Secure" ? "border-emerald-200 bg-emerald-50 shadow-none" : mastery === "Developing" ? "border-amber-200 bg-amber-50 shadow-none" : "shadow-none"}><CardHeader><CardDescription>Mastery</CardDescription><CardTitle className="font-serif text-2xl">{mastery}</CardTitle></CardHeader></Card></div></section>
      ...
      {history.length > 0 && <Card className="shadow-none"><CardHeader><CardTitle className="font-serif text-2xl">Intervention history</CardTitle><CardDescription>Repeated retests show whether an improvement is holding rather than appearing once by chance.</CardDescription></CardHeader><CardContent className="grid gap-3 md:grid-cols-2 lg:grid-cols-4">{history.slice(0, 8).map((item, index) => { const delta = item.retestAccuracy - item.originalAccuracy; return <div key={item.id} className="rounded-xl border bg-white p-4"><div className="flex items-center justify-between"><Badge variant="outline">Retest {history.length - index}</Badge><span className={`text-sm font-bold ${delta >= 0 ? "text-emerald-700" : "text-amber-700"}`}>{delta >= 0 ? "+" : ""}{delta}</span></div><p className="mt-2 font-serif text-2xl font-bold">{item.retestAccuracy}%</p><p className="text-xs text-slate-500">{new Date(item.date).toLocaleDateString("en-GB")}</p></div>})}</CardContent></Card>}
