...
export default function AdvancedPracticePage() {
  ...
  const allForTest = useMemo(() => [...questionBank, ...advancedQuestionBank].filter(q => q.test===test), [test])
  ...
  const [seed,setSeed] = useState(1), [index,setIndex] = useState(0), [selected,setSelected] = useState<number|null>(null), [checked,setChecked] = useState(false), [correct,setCorrect] = useState(0), [attempted,setAttempted] = useState(0), [saved,setSaved] = useState<string[]>([])
  ...
  return <main className="min-h-screen bg-slate-50 text-slate-950">
    ...
    <div className="mx-auto max-w-6xl space-y-6 px-4 py-8">
      <section className="grid gap-5 lg:grid-cols-[1.2fr_.8fr]"><div><p className="mb-2 text-xs font-bold uppercase tracking-[.18em] text-blue-700">Challenge Ladder</p><h1 className="font-serif text-4xl font-bold tracking-tight">Practise harder questions in a deliberate progression.</h1><p className="mt-3 max-w-3xl text-base leading-relaxed text-slate-600">Each ladder selects six original questions from one section and moves Foundation → Stretch → Challenge. The advanced bank adds linked passages, experimental reasoning and multi-step problems rather than only one-step recall.</p></div><Card className="shadow-none"><CardHeader><CardTitle className="font-serif text-xl">Bank expansion</CardTitle><CardDescription>New advanced questions in addition to the existing bank.</CardDescription></CardHeader><CardContent className="grid grid-cols-2 gap-3"><div><p className="text-3xl font-bold">{advancedQuestionBankStats.total.toLocaleString()}</p><p className="text-xs text-muted-foreground">new advanced questions</p></div><div><p className="text-3xl font-bold">{allForTest.length.toLocaleString()}</p><p className="text-xs text-muted-foreground">available for {test}</p></div></CardContent></Card></section>
